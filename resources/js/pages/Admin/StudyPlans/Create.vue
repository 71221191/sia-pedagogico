...
const form = useForm({
    ...
    name: '',
    resolution_code: '',
    evaluation_type: 'vigesimal', // Valor por defecto
    is_active: true, // Por defecto activo
    valid_from_year: new Date().getFullYear(), // Año actual por defecto
    valid_to_year: null, // Nulo por defecto
});

const currentYear = new Date().getFullYear();
const years = computed(() => {
    const arr = [];
    for (let i = currentYear - 20; i <= currentYear + 5; i++) { // Rango de años para seleccionar
        arr.push(i);
    }
    return arr;
});
...
const submit = () => {
    form.post(route('admin.study_plans.store'), {
        onSuccess: () => {
...
<template>
    <Head title="Crear Plan de Estudio" />
    ...
    <div class="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="mb-10">
                ...
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Crear Nuevo Plan de Estudio</h1>
                <p class="mt-2 text-gray-600">Define las características del plan de estudio.</p>
            </div>
