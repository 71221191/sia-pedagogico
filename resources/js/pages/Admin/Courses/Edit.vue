...
const props = defineProps<{
    ...
    cycles: string[]; // Lista de ciclos
    courseTypes: string[]; // Lista de tipos de curso
}>();

const form = useForm({
    _method: 'put', // Importante para la actualización en Laravel
    study_plan_id: props.course.study_plan_id,
    name: props.course.name,
    code: props.course.code,
    cycle: props.course.cycle,
    credits: props.course.credits,
    hours_theory: props.course.hours_theory,
    hours_practice: props.course.hours_practice,
    type: props.course.type,
    is_legacy: props.course.is_legacy,
});
...
<template>
    <Head :title="`Editar Curso: ${course.name}`" />
    ...
    <div class="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
        <div class="max-w-3xl mx-auto">
            ...
            <div class="mb-10">
                ...
                <h1 class="text-3xl font-bold text-gray-900 tracking-tight">Editar Curso: <span class="text-blue-600">{{ course.name }}</span></h1>
                <p class="mt-2 text-gray-600">Modifica los detalles del curso seleccionado.</p>
            </div>
